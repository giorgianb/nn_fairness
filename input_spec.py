...
class InputSpecification:
    def __init__(self, *variables):
        ...
        for i, v in enumerate(variables):
            ...
            if isinstance(v, Continuous):
                ...
                if v.label not in self.one_hot_indices:
                    self.one_hot_indices[v.label] = []

                self.one_hot_indices[v.label].append(i)
    ...
    def get(self, **kwargs):
        ...
        one_hot_specification = dict(self.one_hot_indices.items())
        for k, v in kwargs.items():
            ...
            if k in self.continuous_indices:
                ...
                one_hot_specification[k] = (self.one_hot_indices[k][v],)
            else:
        ...
        return InputClass(self, tuple(boxes))
...
class InputClass:
    ...
    def __and__(self, other):
        ...
        return InputClass(self.specification, boxes)
    ...
    def __or__(self, other):
        ...
        return InputClass(self.specification, tuple(self.boxes) + tuple(other.boxes))
    ...
    def __contains__(self, X):
        return all(self._in_box(X, box) for box in self.boxes)

    @staticmethod
    def _in_box(X, box):
        return len(X) == len(box) and all(box[i, 0] <= X[i] <= box[i, 1] for i in range(len(X)))
